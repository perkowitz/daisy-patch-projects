...
u8 Momo::ctrlParams[MIDI_CC_COUNT] = { PARAM_SAW, PARAM_PULSE, PARAM_SUB, PARAM_SAW2, PARAM_FREQ, PARAM_RES, PARAM_F_FENV, PARAM_FD  };

float Momo::presets[][Momo::PARAM_COUNT] = {        // params are in order of PARAM constant in .h
    {
...
void Momo::Init(float sampleRate) {
    Init(sampleRate, VOICE_COUNT);
}

void Momo::Init(float sampleRate, u8 voiceLimit) {
    ...
    params[PARAM_DRIFT_RATE].Init("Rate", 8, 0, MOMO_MAX_DRIFT_RATE, Parameter::LINEAR, 1);
    ...
    pages[p++].Init(Name(), "Filt", &params[PARAM_FREQ], &params[PARAM_RES], &params[PARAM_HPF], nullptr);
...
float Momo::Process() {
    ...
    klokCount = (klokCount + 1) % MOMO_KLOK_COUNT_LIMIT;
    ...
    if (klokCount % 8 != 0) {     // since klok=3, we process 1 cycle out of 8
    ...
    signal = vcf.Process(signal);
     ...
     // leftSignal = hpf.Process(signal) / voiceLimit;
...
float Momo::GetOutput(u8 channel) {
    switch (channel) {
...
void Momo::NoteOn(u8 note, float velocity) {
...
void Momo::AllNotesOff() {
    AllVoicesOff();
}

void Momo::NoteOff(u8 note) {
    for (u8 voice = 0; voice < voiceLimit; voice++) {
...
void Momo::Clock(u8 measure, u8 step, u8 tick) {
...
ParamPage *Momo::GetParamPage(u8 page) {
    if (page < PAGE_COUNT) {
        return &pages[page];
    }
    return nullptr;
...
void Momo::ResetParams(u8 page) {
    if (page < PAGE_COUNT) {
        paramSets[page].ResetParams();
    }
}

Param *Momo::GetParam(u8 index) { 
...
void Momo::MidiController(u8 cc, u8 value) { 
...
void Momo::LoadPreset(u8 preset) { 
